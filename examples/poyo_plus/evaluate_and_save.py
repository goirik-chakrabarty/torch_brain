...
# --- Added DataSavingEvaluator Class ---
class DataSavingEvaluator(MultiTaskDecodingStitchEvaluator):
    """
    Extends MultiTaskDecodingStitchEvaluator to save predictions and targets
    as .npz files for later visualization.
    """

    def __init__(self, metrics, save_dir="saved_predictions"):
        super().__init__(metrics)
        self.save_dir = save_dir
        self.saved_data = (
            {}
        )  # Structure: {session_id: {task_name: {timestamps, preds, targets}}}
    ...
    def flush_cache(self, i, session_id):
        """
        Overrides the original flush_cache to intercept and store stitched data.
        """
        # First, we process the cache to get stitched values
        for task_name in self.cache[i]["pred"].keys():
            ...
            # Pool data wherever timestamps overlap using the stitch utility
            # We capture unique_timestamps here which are returned by stitch()
            unique_timestamps, stitched_pred = stitch(timestamps, pred)
            _, stitched_target = stitch(timestamps, target)

            if target.dtype == torch.long:
                stitched_target = torch.round(stitched_target).long()

            # --- Custom Saving Logic ---
            if session_id not in self.saved_data:
                self.saved_data[session_id] = {}
            if task_name not in self.saved_data[session_id]:
                self.saved_data[session_id][task_name] = {
                    "timestamps": [],
                    "preds": [],
                    "targets": [],
                }

            # Append to our storage (convert to numpy/cpu immediately to save GPU memory)
            self.saved_data[session_id][task_name]["timestamps"].append(
                unique_timestamps.cpu().numpy()
            )
            self.saved_data[session_id][task_name]["preds"].append(
                stitched_pred.cpu().numpy()
            )
            self.saved_data[session_id][task_name]["targets"].append(
                stitched_target.cpu().numpy()
            )
            # ---------------------------

            # Standard Metric Update (from original class to keep metric logging working)
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        # 1. Run standard metric computation and logging
        super().on_test_epoch_end(trainer, pl_module)

        # 2. Save collected data to disk
        print(f"\nSaving collected data to {self.save_dir}...")
        for session_id, tasks in self.saved_data.items():
            for task_name, data in tasks.items():
                # Concatenate all stitched segments for this session/task
                if not data["timestamps"]:
                    continue

                timestamps = np.concatenate(data["timestamps"])
                ...
                filename = os.path.join(self.save_dir, f"{session_id}_{task_name}.npz")
                np.savez_compressed(
                    filename, timestamps=timestamps, preds=preds, targets=targets
                )
...
def main(cfg: DictConfig):
    ...
    # Replaced standard evaluator with DataSavingEvaluator
    evaluator = DataSavingEvaluator(
        metrics=data_module.get_metrics(),
        save_dir="./model_predictions",  # You can change this path
    )
    ...
    # Test
    checkpoint_pre_path = "/mnt/vast-react/projects/neural_foundation_model/torch_brain_polly_export/model_checkpoints/checkpoints/"
    # '/scratch-grete/projects/nim00012/adapted_torch_brain/torch_brain/examples/poyo_plus/checkpoints/' < -- OLD PATH
    trainer.test(
        wrapper,
        data_module,
